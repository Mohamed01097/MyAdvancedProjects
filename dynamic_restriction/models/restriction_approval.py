...
class DynamicRestrictionApproval(models.Model):
    ...
    def _log_approval_review(self, action_name, reason):
        self.ensure_one()
        log_model_exists = self.env['ir.model'].sudo().search([
            ('model', '=', 'dynamic.restriction.log'),
        ], limit=1)
    ...
    def action_reject(self):
        ...
        for approval in self:
            ...
            approval._log_approval_review(
                'approval_rejected',
                approval.rejection_reason or _('Approval request rejected.'),
            )
        return True

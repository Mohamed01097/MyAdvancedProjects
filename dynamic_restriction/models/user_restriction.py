...
class UserRestriction(models.Model):
    ...
    @api.model
    def _get_applicable_restriction_domain(self, model_name, uid=None, company_id=None):
        uid = uid or self.env.uid
        company_id = company_id or self.env.company.id
        group_ids = self.env['res.users'].sudo().browse(uid).all_group_ids.ids
        group_domain = [('group_ids', 'in', group_ids)] if group_ids else [('id', '=', 0)]
        scope_domain = Domain.OR([
            ['&', ('user_ids', '=', False), ('group_ids', '=', False)],
            [('user_ids', 'in', [uid])],
            group_domain,
        ])
    ...
    def _get_evaluated_domain(self):
        ...
        try:
            domain = safe_eval(self.domain_force)

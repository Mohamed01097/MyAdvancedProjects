...
class BaseModel(models.AbstractModel):
    ...
    def _get_dynamic_restriction_candidate_ids(self, uid, model_name, action_name, company_id):
        ...
        restrictions = Restriction.search(
            Restriction._get_applicable_restriction_domain(
                model_name,
                uid=uid,
    ...
    def _apply_dynamic_own_domain(self, domain):
        ...
        if not own_domain:
            return domain or []
        return Domain.AND([domain or [], own_domain])

    def _check_dynamic_own_records(self, action_name='edit'):
        ...
        for restriction in self._get_dynamic_own_restrictions():
            ...
            if owned_count != len(self):
                ...
                self._log_dynamic_restriction(restriction, action_name, reason, self)
                raise AccessError(
    ...
    def _dynamic_restriction_action_label(self, action_name):
        if action_name in MASS_ACTION_LABELS:
            return _(MASS_ACTION_LABELS[action_name])
        return {
            'create': _('create'),
            'edit': _('edit'),
            'delete': _('delete'),
    ...
    def _dynamic_restriction_matches_records(self, restriction, action_name):
        ...
        return bool(
            self.sudo().search(
                Domain('id', 'in', self.ids) & Domain(domain),
                limit=1,
            )
        )
    ...
    def _log_dynamic_restriction(self, restriction, action_name, reason, records=None):
    ...
    def _raise_dynamic_access_error(self, restriction, action_name, reason, records=None, message=None):
        self._log_dynamic_restriction(restriction, action_name, reason, records)
    ...
    def _raise_dynamic_approval_required(self, restriction, action_name, vals=None):
        ...
        with self.env.registry.cursor() as cr:
            ...
            approval_env['dynamic.restriction.approval'].sudo()._create_requests(
                approval_restriction,
                approval_records,
                action_name,
                vals=vals,
    ...
    def write(self, vals):
        ...
        active_field = self._active_name if self._active_name in vals else 'active'

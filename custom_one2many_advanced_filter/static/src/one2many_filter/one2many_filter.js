...
function allowRendererFilterStateProp(Renderer = ListRenderer) {
    const props = Renderer?.props;
    if (Array.isArray(props)) {
        if (
            !props.includes("*") &&
            !props.includes(O2MAF_RENDERER_PROP) &&
            !props.includes(O2MAF_RENDERER_PROP.slice(0, -1))
        ) {
            Renderer.props = [...props, O2MAF_RENDERER_PROP];
        }
    } else if (props && typeof props === "object" && !props["*"] && !props.o2mafFilterState) {
        Renderer.props = {
            ...props,
            o2mafFilterState: { type: Object, optional: true },
        };
    }
}

allowRendererFilterStateProp();
...
patch(X2ManyField.prototype, {
    ...
    get rendererProps() {
        ...
        if (this.o2mafCanDisplay) {
            allowRendererFilterStateProp(this.constructor.components?.ListRenderer);
